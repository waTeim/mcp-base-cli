"""
MCP Base - CLI tools for MCP server setup and management.

This package provides command-line tools for:
- Setting up Auth0 for MCP authentication (mcp-setup-auth0)
- Creating Kubernetes secrets (mcp-create-secrets)
- Generating configuration files (mcp-make-config)
- Setting up Kubernetes RBAC (mcp-setup-rbac)
- Adding users to allowed clients (mcp-add-user)
"""

__version__ = "0.1.0"
__author__ = "MCP Base Contributors"

__all__ = [
    "__version__",
]
